  ...
  const handleSecureSend = async (e) => {
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        data = { error: text || 'Transmission Failed. Please try again.' };
      }
      ...
      if (response.ok) {
        ...
        setConsoleLogs((prev) => [
          ...
          `[SUCCESS] ${data.message || 'Successfully Transmitted'}`
        ]);
        setFormData({ name: '', email: '', subject: '', message: '' });
      } else {
        throw new Error(data.message || data.error || 'Transmission Failed. Please try again.');
      }
    } catch (error) {
      setIsSending(false);
      setPacketStatus('INTEGRITY_COMPROMISED');
      setConsoleLogs((prev) => [
        ...prev.slice(-4),
        '[ERROR] Connection intercepted or dropped.',
        `[ERROR] ${error.message || 'Transmission Failed. Please try again.'}`
      ]);
    }
  };
